...
inductive_set addDataDep :: "time_type set  \<Rightarrow> (time_type \<Rightarrow> ('tid \<times> aid_type \<times>
 ...
 additionalDep : "\<lbrakk> (a,b) \<in> trancl sb; rho a = Some (tida,aida,aca);
                    (rho b) = Some (tidb,aidb,acb); {a,b} \<subseteq> E; isWrite aca \<or> isRead aca;
                                isWrite acb \<or> isRead acb; getDepSet acb = Some al; aida \<in> set al\<rbrakk>
...
| fromSeqFence : "\<lbrakk> a < b; rho a = Some (tid,aid,ac);  rho b = Some (tid1,aid1,Fence SeqCst);
                   isWrite ac;  acLoc ac = Some x; \<not> seqFenceInMid E rho a b;
                   \<not> writeInMid T rho a b x\<rbrakk> \<Longrightarrow> (a,x,b) \<in> cw E rho rf"
| seqFenceInduct : "\<lbrakk> (a,x,b') \<in> cw E rho rf; isSeqFence (rho b');
                       isWriteInTuple (rho a); getLocInTuple (rho a) = Some x;
                       \<not> seqFenceInMid E rho b' b; \<not> writeInMid T rho b' b x\<rbrakk>
...
(* first, we have single thread write-read not relation *)
inductive_set singleCocw :: "time_type set \<Rightarrow> (time_type \<Rightarrow> ('tid \<times> aid_type \<times>
                   (aid_type, 'val, loc_type, 'lock, 'name, 'callID) action) option) \<Rightarrow>
              (time_type \<times> time_type) set
          \<Rightarrow> (time_type \<times> time_type) set"
  for E :: "time_type set"
    and rho :: "(time_type \<Rightarrow> ('tid \<times> aid_type \<times>
                   (aid_type, 'val, loc_type, 'lock, 'name, 'callID) action) option)"
    and rf :: "(time_type \<times> time_type) set" where
 singleCocws : "\<lbrakk>(a,b) \<in> rf \<and> sameThread rho a b; getLocInTuple (rho a) = Some x;
                 (\<exists> t . t \<in> E \<and> a < t \<and> t < b
                       \<and> sameThread rho a t \<and> getLocInTuple (rho t) = Some x)\<rbrakk> 
                    \<Longrightarrow> (a,b) \<in> singleCocw E rho rf"
...
inductive_set crossCocw :: "time_type set \<Rightarrow> (time_type \<Rightarrow> ('tid \<times> aid_type \<times>
  ...
  for E :: "time_type set"
    and rho :: "(time_type \<Rightarrow> ('tid \<times> aid_type \<times>
                   (aid_type, 'val, loc_type, 'lock, 'name, 'callID) action) option)"
    and rf :: "(time_type \<times> time_type) set" where
 crossCocw1 : "\<lbrakk>(a,b) \<in> rf \<and>
                 (\<exists> t t' . t \<in> E \<and> t' \<in> E \<and> t < a \<and> b < t' \<and>
                      sameLocInTuple rho t a \<and> sameLocInTuple rho b t'
                     \<and> isSeqInTule (rho t) \<and> (t',t) \<in> rf)\<rbrakk> 
                    \<Longrightarrow> (a,b) \<in> crossCocw E rho rf"
|  crossCocw2 : "\<lbrakk>(a,b) \<in> rf \<and>
                 (\<exists> t t' . t \<in> E \<and> t' \<in> E \<and> a < t \<and> t' < b \<and>
                      sameLocInTuple rho t a \<and> sameLocInTuple rho b t'
                     \<and> isSeqInTule (rho t) \<and> (t',t) \<in> rf)\<rbrakk> 
                    \<Longrightarrow> (a,b) \<in> crossCocw E rho rf"

definition cocw where
"cocw E rho rf = singleCocw E rho rf \<union>  seqCocw E rho rf \<union> crossCocw E rho rf"
...
definition satCreation where
"satCreation T rho = (\<forall> a b . a \<in> T \<and> b \<in> T \<and> sameLocInTuple rho a b \<and> isCreate (rho a) 
...
"sat Tid T rho po rf = (rhoAssumption rho \<and> rfAllAssumption rf rho
                    \<longrightarrow> (satCreation T rho \<and> satE Tid T rho po \<and> (\<forall> (a,b) \<in> rf . (a,b) \<notin> cocw T rho rf)))"
...
fun raceDef where
"raceDef T rho = (\<forall> t1 t2 tid aid1 aid2 v v' x t n axl vol . t1 \<in> T \<and> t2 \<in> T \<and> rho t1 = Some (tid,aid1, NRead vol v x t 1 n axl)
