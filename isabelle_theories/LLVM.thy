...
theory LLVM
imports trans_flowgraph AxiomaticModel
begin
...
locale LLVM_MM = axiomaticModel where
     actions = "actions :: (time_type, 'val, loc_type, 'lock, 'name, 'callID) action set"
  ...
  for actions and locations and actionIDs and times and threads and locks and names and callIDs
...
+ fixes free_set :: "loc_type set \<Rightarrow> nat \<Rightarrow> (loc_type \<times> loc_type set)"
assumes free_set_property : "\<forall> S n start nexts . free_set S n = (start, nexts)
                      \<and> finite S \<longrightarrow> finite nexts \<and> start \<in> nexts \<and> next \<inter> S = {}"
begin
...
| atomic_store [intro]: "\<lbrakk>L p = Atomic_store vol ty1 e1 ty2 e2 or1;eval_expr env  e2 = CPointer l; p \<noteq> Exit\<rbrakk> \<Longrightarrow> 
 LLVM_step tid mem (_,p, env, stack,alloced, lockMap) {AWrite vol (eval_expr env e1) l (tranOrder or1) []}
        (p,next_node Edges seq p, env, stack, alloced,lockMap)"
| malloc [intro] : "\<lbrakk>L P = Malloc x ty;free_set alloced (LLVM_sizeof ty) = (v,nexts); p \<noteq> Exit\<rbrakk> \<Longrightarrow>
