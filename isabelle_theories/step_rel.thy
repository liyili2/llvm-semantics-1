...
step_single [intro!]: "\<lbrakk>step_rel tid t G mem C ops C'; update_mem mem t ops mem'\<rbrakk> \<Longrightarrow> 
  one_step tid G (t,C, mem) (t+1,C', mem')"
...
step_thread [intro]: "\<lbrakk>CFGs tid = Some G; states tid = Some C; step_rel tid t G mem C ops C'; 
  update_mem mem t ops mem'\<rbrakk> \<Longrightarrow> conc_step CFGs (t,states, mem) (t+1,states(tid \<mapsto> C'), mem')"
...
lemma run_prog_step: "\<lbrakk>run_prog CFGs (t,S, mem); S tid = Some s; CFGs tid = Some G;
 step_rel tid t G mem s ops s'; update_mem mem t ops mem'\<rbrakk> \<Longrightarrow> run_prog CFGs (t+1,S(tid \<mapsto> s'), mem')"
  apply (erule run_prog_one_step, auto)
  using step_single by auto
